...
namespace ExchangeSharp
{
	...
	public sealed partial class ExchangeBTSEAPI : ExchangeAPI
	{
		...
		protected override async Task<Dictionary<string, decimal>> OnGetFeesAsync()
		{
			...
			var result = await MakeJsonRequestAsync<JToken>("/api/v3.1/TestModel/fees",
			...
			string? marketSymbol = null)
		{
			...
			var url = new UriBuilder(BaseUrl) {Path = "/api/v3.1/TestModel/open_orders"};
			url.AppendPayloadToQuery(new Dictionary<string, object>()
			{
				{"symbol", marketSymbol}
			});

			var result = await MakeJsonRequestAsync<JToken>("/api/v3.1/TestModel/open_orders"+url.Query,
				requestMethod: "GET", payload: payload);
		...
		private async Task<Dictionary<string, decimal>> GetBTSEBalance(bool availableOnly)
		{
			...
			var result = await MakeJsonRequestAsync<JToken>("/api/v3.1/TestModel/wallet",
				requestMethod: "GET", payload: payload);
